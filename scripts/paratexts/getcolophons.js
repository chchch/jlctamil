...
const xsltSheet = fs.readFileSync('scripts/paratexts/xslt/tei-to-html-reduced.json',{encoding:'utf-8'});

//const dir = '../../../mss/';
const dir = 'scripts/paratexts/mss/';
...
const readfiles = function(arr) {
    const tab = arr.map((f) => 
    {
        ...
        const colophons = getColophons(xmlDoc);
        return {
            cote: cote,
    ...
    */
    const template = new jsdom.JSDOM(fs.readFileSync('colophons-template.html',{encoding:'utf8'})).window.document;
    const table = template.querySelector('#index').firstElementChild;
    const tstr = tab.reduce((acc, cur) => {
        if(cur.colophons.length > 0) {
            const lines = [...cur.colophons].reduce((acc2,cur2) => fileredux(acc2,cur2,cur),'');
            return acc + lines;
        }
        else return acc;
    },'');
    const thead = '<tr id="head"><th>Colophon</th><th class="sorttable_alphanum">Shelfmark</th><th>Repository</th><th>Title</th></tr>';
