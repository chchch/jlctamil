...
const readfiles = function(arr) {
    ...
    */
    const template = new jsdom.JSDOM(fs.readFileSync('tbc-template.html',{encoding:'utf8'})).window.document;
    const table = template.querySelector('#index').firstElementChild;
    const tstr = tab.reduce((acc, cur) => {
        if(cur.tbc.length > 0) {
            const lines = [...cur.tbc].reduce((acc2,cur2) => fileredux(acc2,cur2,cur),'');
            return acc + lines;
        }
        else return acc;
    },'');
    const thead = '<tr id="head"><th>Paratext</th><th class="sorttable_alphanum">Shelfmark</th><th>Repository</th><th>Title</th><th>Unit</th><th>Page/folio</th><th>Placement</th></tr>';
    table.innerHTML = thead + tstr;
    fs.writeFile('tbc.html',template.documentElement.outerHTML,{encoding: 'utf8'},function(){return;});
};
